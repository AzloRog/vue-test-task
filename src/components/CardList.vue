    ...
    <div class="sort">
      <v-tooltip text="Сортировка по возрастанию рейтинга">
        <template v-slot:activator="{ props }">
          <v-btn v-bind="props" icon="mdi-sort-ascending" density="compact" variant="tonal" class="sort-btn"
            :style="`background: ${options.color}`" @click="() => sortList('increaseRating')" />
        </template>
      </v-tooltip>
      <v-tooltip text="Сортировка убыванию рейтинга">
        <template v-slot:activator="{ props }">
          <v-btn v-bind="props" icon="mdi-sort-descending" density="compact" variant="tonal" class="sort-btn"
            :style="`background: ${options.color}`" @click="() => sortList('decreaseRating')" />
        </template>
      </v-tooltip>
      <v-tooltip text="Без сортировки">
        <template v-slot:activator="{ props }">
          <v-btn v-bind="props" icon="mdi-sort-alphabetical-ascending" density="compact" variant="tonal"
            class="sort-btn" :style="`background: ${options.color}`" @click="() => sortList('default')" />
        </template>
      </v-tooltip>
...
function sortList(sortType) {
  switch (sortType) {
    ...
    case "decreaseRating":
      ...
      break;
    default:
      cards.value.sort((a, b) => a.title.localeCompare(b.title))
      break;
